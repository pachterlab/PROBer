...
public:
	...
	*/
	...
	/*
		...
		@param   pos   leftmost position from 5' end, 0-based
		@param   fragment_length  fragment length, 0 means SE read 
		@return   conditional probability of generating such a read given the isoform
	*/
	double getConProb(int tid, int pos = 0, int fragment_length = 0) const {
		assert(tid >= 0 && tid <= M);
		if (tid == 0) return 1.0;
		return (fragment_length > 0 ? transcripts[tid]->getProb(pos, fragment_length) : transcripts[tid]->getProb(pos));
	}
