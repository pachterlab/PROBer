...
void init() {
  ...
  whole_model = new PROBerWholeModel(configF, (!strcmp(channel, "minus") ? 0 : 1), &transcripts, num_threads, read_length, isMAP);
  if (!strcmp(channel, "plus")) whole_model->read(sampleName); // Read gamma because this run is used to estimate betas

  // Create PROBerReadModel
  read_model = new PROBerReadModel(model_type, &refs, read_length);
  ...
  for (int i = 0; i < num_threads; ++i) 
    paramsVec[i]->estimator = new PROBerReadModel(read_model);
  whole_model->init();
...
void EM() {
  ...
  do {
    ...
    count0 = N0;
    if (N0 > 0) curr_logprob += N0 * log(whole_model->getTheta(0));
    curr_logprob += read_model->calcLogP();
    for (int i = 0; i < num_threads; ++i) {
    ...
    // Run PROBerWholeModel's runEM procedure
