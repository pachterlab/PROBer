...
bool stochastic; // Turn on stochastic EM for SE or partially aligned PE reads
...
int main(int argc, char* argv[]) {
  ...
  if (argc < 5) {
    printf("Usage : rsem-run-em refName sampleName imdName statName [-p #Threads] [--stochastic] [--gibbs-out] [-b] [--sampling] [--seed seed] [-q]\n\n");
    ...
    printf("  --stochastic: use stochastic EM for SE reads or partial aligned PE reads. (default: off)\n");
  ...
  for (int i = 5; i < argc; i++) {
    ...
    if (!strcmp(argv[i], "--stochastic")) stochastic = true;
    if (!strcmp(argv[i], "-b")) genBamF = true;
